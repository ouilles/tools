...
if [ ! -d ./portapack-mayhem-1.5.0 ]; then
	echo "--- Cloning Mayhem Portapack repository from GitHub ---"
	sleep 1
	mkdir -p portapack-mayhem-1.5.0
	cd ./portapack-mayhem-1.5.0
	git clone -b v1.5.0 --recurse-submodules https://github.com/eried/portapack-mayhem.git .
	# USER=`whoami`
	# GROUP=`id -g` 
