...
if [ ! -d ./portapack-mayhem-1.4.4 ]; then
	echo "--- Cloning Mayhem Portapack repository from GitHub ---"
	sleep 1
	mkdir -p portapack-mayhem-1.4.4
	cd ./portapack-mayhem-1.4.4
	git clone -b v1.4.4 --recurse-submodules https://github.com/eried/portapack-mayhem.git .
	# USER=`whoami`
	# GROUP=`id -g` 
